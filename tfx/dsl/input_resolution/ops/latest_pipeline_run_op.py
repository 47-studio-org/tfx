...
class LatestPipelineRun(
  ...
  def apply(self) -> typing_utils.ArtifactMultiMap:
    ...
    if not self.pipeline_name:
      raise ValueError(
          'pipeline_name for LatestPipelineRunOp can not be empty.')
    ...
    # Gets the COMPLETE executions of the pipeline end node, and then sort them
    # from new to old.
    ...
    if not pipeline_end_node_executions:
      raise exceptions.SkipSignal(
          f'Pipeline {self.pipeline_name} does not have a successful exeuction.'
      )
    pipeline_end_node_executions.sort(
        key=lambda e: (e.create_time_since_epoch, e.id), reverse=True)

    all_artifacts_of_end_nodes = self.context.store.get_artifacts_by_context(
        context_id=pipeline_end_node_ctx.id)
    if not all_artifacts_of_end_nodes:
      raise exceptions.SkipSignal(
          f'Pipeline {self.pipeline_name} does not have any output artifacts '
          'from PipelineEnd node.')

    # From the newest pipeline end node execution to oldest pipeline end node
    # execution, if we can find some artifacts from the execution, returns the
    # artifacts of the execution.
    for execution in pipeline_end_node_executions:
      ...
      key_to_artifact_ids = {}
      for event in end_node_output_events:
        output_key = event_lib.get_artifact_path(event)
        key_to_artifact_ids.setdefault(output_key[0],
                                       []).append(event.artifact_id)

      result = {}
      for key, ids in key_to_artifact_ids.items():
        artifact_protos = [a for a in all_artifacts_of_end_nodes if a.id in ids]
    ...
    raise exceptions.SkipSignal(
        'No artifact is returned by LastestPipelineRunOp.')
